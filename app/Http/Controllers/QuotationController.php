...
class QuotationController extends Controller
{
    ...
    public function store_view(Request $Request)
    {
        $data = null;
        if ($Request->target) {
            $data = _Request::findorfail($Request->target);
        }
    ...
    public function patch_action(Request $Request, $id)
    {
        ...
        if ($Request->delete && !$Request->product && $Quotation->Items->count() == count($Request->delete)) {
            return Redirect::back()->withInput()->with([
        ...
        QuotationItem::where('quotation', $Quotation->id)->delete();
