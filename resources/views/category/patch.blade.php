...
@section('title', ucwords(__('Update Category')) . ' #' . $data->id)

@section('content')
    <div class="flex flex-col gap-4">
        <div class="w-full flex items-center justify-between gap-2">
            <h1 class="font-x-core text-2xl">{{ ucwords(__('Update Category')) }} #{{ $data->id }}</h1>
            <div
                ...
                <div class="container mx-auto lg:w-max p-4 lg:p-0">
                    <div
                        ...
                        <form action="{{ route('actions.categories.clear', $data->id) }}" method="POST">
                            @csrf
                            @method('delete')
                            <button type="submit"
        ...
        <div class="toremove bg-x-white rounded-x-core shadow-x-core p-4">
            <form id="form" action="{{ route('actions.categories.patch', $data->id) }}" method="POST"
                ...
                <div class="w-full flex flex-col gap-px max-w-[300px] mx-auto">
                    ...
                    <div id="image_display"
                        ...
                        <img src="{{ Core::files(Core::CATEGORY)->get($data->file) }}"
...
@section('scripts')
    <script>
        ...
        document.addEventListener("DOMContentLoaded", () => {
            document.querySelector("#image_display").classList.remove("hidden");
            document.querySelector("#image_display").classList.add("flex");
        });
