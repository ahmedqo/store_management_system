...
@section('header')
    <div class="w-full p-4 container mx-auto">
        <div style="background: radial-gradient(var(--acent), var(--prime))" class="w-full rounded-x-core overflow-hidden">
            <div style="text-shadow: 0px 3px 12px #1d1d1d50, #1d1d1d25 0px 25px 20px; background-image: url({{ asset('img/bg-3.webp') }})"
...
@section('content')
    <section class="w-full flex flex-col lg:flex-row lg:flex-wrap gap-4 lg:gap-6 p-4">
        ...
        <div class="w-full grid grid-rows-1 grid-cols-1 lg:grid-cols-4 gap-4 lg:gap-6 items-start">
            @include('shared.guest.categories', [
                'categories' => $categories,
            ])
            <div class="w-full lg:col-span-3 grid grid-rows-1 grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
                @if (request()->search)
                    <h3 class="w-full col-span-2 lg:col-span-3 text-x-black font-x-core text-lg lg:text-xl -mb-2 lg:-mb-4">
                ...
                @empty
                    <h6 class="text-x-black p-4 text-xl font-x-core uppercase text-center col-span-2 lg:col-span-3">
                        {{ __('No data found') }}
                    </h6>
                @endforelse
            </div>
