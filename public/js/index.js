    ...
    JSON.parse(localStorage[data]).forEach((row) => {
        _target.innerHTML += `
            <div class="w-full flex flex-wrap gap-4 items-center">
                ...
                <a href="${row.link}" class="h-full w-20 aspect-square bg-blue-500 rounded-md row-span-2 overflow-hidden">
...
function QuotationRow(data, idx = false) {
    ...
    tr.innerHTML = `
        <td class="text-x-black text-sm font-x-core p-2">
            ${document.querySelector("tbody").children.length + 1}
...
QuotationRow.Total = function() {
    document.querySelector("#subtotal").innerHTML = format([...document.querySelectorAll(".lot")].reduce((s, e) => s + +e.innerHTML.replace(",", ""), 0));
    document.querySelector("#total").innerHTML = format(+document.querySelector("#subtotal").innerHTML.replace(",", "") + +document.querySelector("#charges").innerHTML.replace(",", ""));
};